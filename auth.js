 ...
 * @returns 
 */
function twitchAuthMiddleware(options) {
    config = options;
    return middleware;
}
...
function middleware(req, res, next) {
    ...
    if (req.path === '/login') return login(req, res, next);
...
function transform(object) {
    return Object.entries(object).map(([key, value]) => `${key}=${encodeURIComponent(value)}`).join('&');
}
...
module.exports = twitchAuthMiddleware;